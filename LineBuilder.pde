class LineBuilder implements BVH3DRayTest
{
  ...
  void appendFacePatternEdges(int boxIndex, int faceIndex)
  {
    ...
    // brighter ones get fewer - computed here, before generation, so the multiplier's
    ...
    data.facepattern.generateWorldEdges(box, boxIndex, faceIndex, shadingMultiplier, frame.camera_pos, worldScratch);
